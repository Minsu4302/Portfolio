...
export const projects: Project[] = [
    ...
    troubleshooting: [
      {
        ...
        solution: '버전별 재현 테스트로 SDK 내부 구조 문제 직접 특정. 프론트 마이그레이션 가이드 작성 및 협업 주도',
  ...
  },
  {
    ...
    status: 'GCP 배포 완료',
    period: '2025',
    summary: 'AI 에이전트 실행 환경과 자기 수정 루프를 함께 설계한 인프라.',
    image: '/icons/harness.svg',
    github: 'https://github.com/Minsu4302/Harness_Infra',
    stack: ['Shell', 'Spring Boot', 'Java 21', 'GCP', 'GitHub Actions', 'Docker', 'Claude Sonnet 4.6', 'Gemini 1.5 Flash', 'GPT-4o-mini', 'RAG', 'CoT', 'PEV Loop'],
    stats: [
      { num: '-75%', label: 'CoT 출력\n토큰 절감' },
      { num: '-98%', label: '세션 이력\n자동 압축' },
      { num: '90%', label: '제약\n자동화율' },
    ...
    background: `LLM 기반 자동 모니터링을 직접 구축하며 AI 에이전트 자체의 컨텍스트 관리·토큰 비용·코드 품질 검증이 수동 의존이라는 새 병목을 발견했습니다. 프롬프트 엔지니어링(무엇을 말할지) → 컨텍스트 엔지니어링(무엇을 볼지) → 하네스 엔지니어링(어디서 작동할지) → 루프 엔지니어링(스스로 수정하는지)까지 4단계로 문제를 분해하고, Harness(로컬 컨텍스트·루프 제어)와 Orchestration(GCP 기반 AI 게이트) 두 레이어로 구조적 해결.`,
    contributions: [
      ...
      { title: '루프 엔지니어링 — 자기 수정 루프 설계', desc: 'L1 PEV 루프: 커밋 전 constraint-check.sh 검증 실패 시 실패 컨텍스트를 재주입하고 최대 3회 재시도. L2 Reflection Agent: 세션 종료 후 태스크·루프 히스토리를 분석해 reflection.md를 갱신, 다음 세션 context-loader.sh에 자동 주입. L3 C10 Loop Convergence: 3회 초과 시 태스크 분할을 강제해 무한 루프를 원천 차단.' },
      { title: '세션 압축 + 제약 자동화', desc: '세션 버퍼 자동 압축으로 10개 세션 이력 600줄 → 9줄 (-98%). C01~C10 린터 10개 중 9개를 커밋·CI에서 자동 차단, 제약 자동화율 90%.' },
    ],
    troubleshooting: [
      {
        title: 'loop-runner.sh C10 수렴 실패 반복',
        problem: 'PEV 루프가 동일한 constraint-check.sh 실패를 3회 넘게 반복. 실패 컨텍스트를 재주입해도 같은 지점에서 계속 막혀 태스크 자체의 범위가 너무 크다는 신호였음.',
        solution: 'loop-runner.sh --status로 재시도 이력을 확인하고 --check로 실패 원인을 수동 재현. C10 규칙에 따라 3회 초과 시 자동으로 태스크 분할을 강제하도록 설계해, 동일 실패를 무한 반복하는 대신 더 작은 단위로 쪼개 재시도하게 함.',
        result: '동일 실패의 무한 재시도 제거. 태스크 분할 후 각 하위 태스크가 1~2회 재시도 내에 PASS하도록 수렴 속도 개선.',
      },
      {
